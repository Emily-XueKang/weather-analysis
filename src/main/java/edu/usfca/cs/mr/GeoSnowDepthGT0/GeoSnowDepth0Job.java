 ...
 */
public class GeoSnowDepth0Job {
    public static void main(String[] args) {
        try {
            Configuration conf = new Configuration();
            // Give the MapRed job a name. You'll see this name in the Yarn
            // webapp.
            Job job = Job.getInstance(conf, "snow depth gt zero job");
            // Current class.
            job.setJarByClass(GeoSnowDepth0Job.class);
            // Mapper
            job.setMapperClass(GeoSnowDepth0Mapper.class);
            // Combiner. We use the reducer as the combiner in this case.
            job.setCombinerClass(GeoSnowDepth0Reducer.class);
            // Reducer
            job.setReducerClass(GeoSnowDepth0Reducer.class);
